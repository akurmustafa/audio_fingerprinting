function [directory] = add_song_to_database(song_path, id)
%	Function to add songs to the database
%   Detailed explanation goes here
    [song, Fs]=audioread(song_path);
    if size(song, 2) > 1
        song_single_channel = (song(:,1)+song(:,2))/2;
    else
        song_single_channel = song;
    end
    % figure();
    % spectrogram(songShort,'yaxis');
    [s ,w, t]=spectrogram(song_single_channel,1024,800,1024,Fs,'yaxis');
...
%         end
    ...
    M = containers.Map(address_decimal,couple_decimal);
    directory = sprintf('song_hashes/%s', num2str(id, '%04i'));
    save(directory,'M');
end
